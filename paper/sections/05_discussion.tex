...
Conee proposed that Mary gains \textit{knowledge by acquaintance}---direct experiential contact with redness, distinct from knowledge-that or knowledge-how. In our framework, this corresponds to the \textbf{Sensory Encoder's output}. The frozen vision encoder always produced chromatic representations; Mary's Conceptual Bridge simply never received them. Upon release, she becomes ``acquainted'' with a signal format that existed all along but was filtered out by her training regime.
...
\begin{table*}[t]
    ...
    \begin{tabular}{@{}llp{5cm}@{}}
        ...
        \hline
        Ability Hypothesis & VQA Performance            & Improved classification after chromatic fine-tuning                  \\
        Acquaintance       & Sensory Encoder output     & Direct exposure to chromatic format (always existed, newly accessed) \\
...
The ``Wow'' signal is what \textit{all} these perspectives miss: the measurable computational cost of format translation at the Conceptual Bridge. This is neither a fact, nor an ability, nor a mode of presentation---it is the \textit{friction} between mismatched representational formats. Perhaps this is what qualia \textit{are}: the metabolic signature of translation.
